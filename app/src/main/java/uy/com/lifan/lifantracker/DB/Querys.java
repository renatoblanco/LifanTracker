...
public class Querys {
    ...
    public static final String QRY_DATOS_VIN = "select  line,z_enddate,z_shipment,(select value from m_product where m_product_id= pp.m_product_id),z_vin,z_engine,z_ecuengine,z_startdate, z_enddate, lifan_keycode,color,(select proceso from effa_li where effa_li_id =(select effa_li_id from effa_li_linea where vin = trim('%s')) ) as proceso from m_productionplan pp where isactive='Y' and z_vin = trim( '%s')";
